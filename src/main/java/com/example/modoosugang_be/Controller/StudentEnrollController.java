...
public class StudentEnrollController {
    ...
    public List LectureTable(@RequestBody Map<String, Object> param) {
        ...
        List<Lecture> logs = lectureService.callUnivLecture(univ);
...
//        String semester = param.get("semester").toString();
        ...
        Optional<Lecture> OptionalLecture = lectureService.findRemain(Integer.valueOf(code));
        ...
        String student_id = univ+"@"+id; // 일단 임의설정


        if (isRemain > 0) {
            int InputRemain = isRemain - 1; // 잔여인원 차감
            lectureService.setRemain(Integer.valueOf(code), InputRemain); // 차감된 인원 DB update
            ...
            regist.setId(Integer.valueOf(code)); // lecture_index : 강의code ?
            regist.setLecture(OptionalLecture.get().getIdx());
            regist.setStudent(student_id);
            regist.setSemester(semester);
            regist.setSchedule(Integer.valueOf(schedule));
            regist.setUniv(univ);
...
//        String code = param.get("code").toString();
        String univ = param.get("univ").toString();
        String semester = param.get("semester").toString();
        String student_id = univ + "@" + param.get("id").toString();
        ...
        for (RegisterLecture log : logs) {
            Optional<Lecture> OptionalLecture = lectureService.findRemain(log.getId());

            JSONObject data = new JSONObject();

            // Create Json Array
            data.put("index", log.getId());
    ...
    public boolean DropLecture(@RequestBody Map<String, Object> param) {
        ...
        Optional<Lecture> OptionalLecture = lectureService.findRemain(Integer.valueOf(code));
        int isRemain = OptionalLecture.get().getRemain();
        int InputRemain = isRemain + 1;
        lectureService.setRemain(code, InputRemain);
